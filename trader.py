...
class Trader:
    def __init__(self,order_manager,balance,holdings):
    ...
    def take_action(self, stock):
        ...
        trading_acc_balance=self.order_manager.trading_acc_balance()
        if(action=='BUY' and trading_acc_balance>=price*self.quantity):
            
            self.order_manager.buy(stock,price,self.quantity,exchange) 
            
        elif(trading_acc_balance<price*self.quantity):
        ...
        elif (action=='SELL' and self.holdings[stock]>=self.quantity):
            self.order_manager.sell(stock,price,self.quantity,exchange)

oms=OrderManagementSystem(50)
available_stocks=['a','b','c']
trader1=Trader(oms,100000000,{'a':2000,'b':4000,'c':5000})
